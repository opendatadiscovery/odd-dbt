...
class GeneratorFabric:
    def __init__(
        self, adapter_type: AdapterType, databases: str, context_profile: dict
    ):
    ...
    def get_generator(self) -> Optional[Generator]:
        ...
        return generator_class(
            host_settings=self.host_settings, databases=self.databases
        )
...
class DbtTestMapper:
    def __init__(
        self, tests_results: list[dict], context: DbtRunContext, generator: DbtGenerator
    ) -> None:
    ...
    def _map_result(self, test_result: dict) -> Optional[tuple[DataEntity, DataEntity]]:
        try:
            ...
            oddrn = self._generator.get_oddrn_by_path(
                "runs", f"{test_id}.{invocation_id}"
            )
    ...
    def _get_dataset_oddrn(self, test_config: dict, nodes: dict):
        ...
        model_name = re.search(
            r"'(.*)'", test_config["test_metadata"]["kwargs"]["model"]
        ).group(1)
        ...
        path = (
            "tables" if model_config["config"]["materialized"] == "table" else "views"
        )

        generator = GeneratorFabric(
            adapter_type, database, self._context.profile
        ).get_generator()
